...
- Three header boxes — Number, Mode, Amount — plus a 4x4 numeric keypad (7/8/9/R, 4/5/6/Tab, 1/2/3/ENTER, Clear/0/00/blank) and a row of 8 pattern-mode tabs: **အပါ, ထိပ်, နောက်, ပါဝါ, ဘရိတ်, နက္ခတ်, ခွေ, အပူး**. The keypad is `position: sticky; bottom: 0` so it stays reachable at the bottom of the screen while the pending-entries list above is scrolled, with slightly shorter keys than the original layout to leave more screen room.
...
- **ပါဝါ (power)**, **နက္ခတ် (nekkhat)**, **အပူး (twin)**: fixed 10-number lists, no digit input needed — just set an amount and ENTER/OK. ပါဝါ = 05,16,27,38,49,50,61,72,83,94. နက္ခတ် = 07,18,24,35,42,53,69,70,81,96. အပူး = 00,11,22,33,44,55,66,77,88,99 (the double/twin numbers).
- **ခွေ (kwe)**: type 3 or more digits — generates every ordered pair of distinct digit positions, walked in original input order, as a 2-digit number (e.g. `123` -> 12,13,21,23,31,32; `2345` -> 23,24,25,32,34,35,42,43,45,52,53,54). Deduped by resulting number string as a safety default for repeated-digit inputs (untested against a worked example, but harmless — a real lottery number should only appear once per submission anyway).
