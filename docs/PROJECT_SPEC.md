...
- ENTER commits to a pending list (editable, per-row delete); the bottom "Submit" button (or OK behavior for fixed-list tabs) opens a confirm dialog stating the count and total before calling `submitBetEntries` — same confirm-then-submit pattern as Bulk Entry, and same recovery behavior (server-rejected rows stay in the list to fix and resubmit).
