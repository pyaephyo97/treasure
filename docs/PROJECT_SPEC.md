...
- Three header boxes — Number, Mode, Amount — plus a 4x4 numeric keypad (7/8/9/R, 4/5/6/Tab, 1/2/3/ENTER, Clear/0/00) and a row of 8 pattern-mode tabs: **အပါ, ထိပ်, နောက်, ပါဝါ, ဘရိတ်, နက္ခတ်, ခွေ, အပူး**. ENTER spans both bottom rows (a taller primary key, like a calculator's Enter) and is colored green. The keypad plus the Cancel/Submit bar right below it are bundled into one `position: sticky; bottom: 0` dock (no gap between them) so the whole control cluster stays reachable at the bottom of the screen while the pending-entries list above (up to `36vh` tall) is scrolled. The dock bleeds through the card's own bottom padding via a negative `marginBottom` (with matching `border-bottom-radius`) so Submit/Cancel sit flush against the card edge instead of leaving a dead gap below them.
