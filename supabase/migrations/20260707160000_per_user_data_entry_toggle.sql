-- ============================================================================
-- Treasure — per-user "close entry" toggle (Account Management)
-- ============================================================================
-- Lets an Admin close/open an individual User's ability to submit bet
-- entries for the CURRENT session, without closing the session for anyone
-- else. Resets to true automatically whenever a new session is opened (see
-- supabase/api.ts openSession).
-- ============================================================================

alter table public.accounts
  add column if not exists data_entry_open boolean not null default true;
...
create policy bet_entries_insert_own on public.bet_entries
  ...
  with check (
    ...
    and amount > 0
    and coalesce(
      (select a.data_entry_open from public.accounts a where a.id = public.current_account_id()),
      true
    )
    and exists (
...
-- Keep submit_bet_entries (the RPC path the UI actually calls) in sync with
-- the same rule, so the error message is friendly instead of a generic RLS
-- violation.
